...
def predict_with_pytorch(model, train_images, train_labels, test_images, test_labels, optimizer_name, optimizer_params, criterion, epochs_num, batch_size):
    ...
    ax1.plot(list(range(epochs_num)), f_scores)
    ax1.plot(list(range(epochs_num)), recalls)
    ax1.plot(list(range(epochs_num)), precisions)
    ax1.legend(['f_scores', 'recalls', 'precisions'])
    ax2 = fig.add_subplot(3, 1, 2)
    ax2.plot(list(range(epochs_num)), accuracies)
    ax2.legend(['accuracies'])
    ax3 = fig.add_subplot(3,1,3)
    ax3.plot(list(range(epochs_num)), losses)
    ax3.legend(['losses'])
    fig.savefig('/home/houda/Workspace/results/test.png')
